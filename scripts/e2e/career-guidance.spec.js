...
test("推荐日程可以从方案确认区一键采用", async ({ page }) => {
  ...
  const option = scheduleLabels
    .nth(scheduleIndex)
    .locator("xpath=ancestor::article[contains(@class, 'career-option')]");
  ...
  await expect(confirmation.locator("dl > div").first().locator("dd"))
    .toHaveText(recommendedLabel);
