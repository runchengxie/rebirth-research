...
test("第一话把目标、调查、日程和研究承诺收敛到可理解范围", async ({ page }) => {
  ...
  // 关键月步骤器：一次只显示一个步骤。
  await expect(page.locator(".decision-step-nav li")).toHaveCount(4);
  await expect(page.locator('.decision-step-nav [aria-current="step"]')).toHaveText("调查");
  ...
  await expect(page.locator(".focus-choice-section")).toHaveCount(0);
  await expect(page.locator(".research-commitment")).toHaveCount(0);

  await page.locator(".decision-step-actions .primary-action").click();
  await expect(page.locator('.decision-step-nav [aria-current="step"]')).toHaveText("日程");
  ...
  await page.locator(".decision-step-actions .primary-action").click();
  await expect(page.locator('.decision-step-nav [aria-current="step"]')).toHaveText("观点");
...
test("窄屏关键月按步骤完成决策并只结算一次", async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await openCareerDecision(page);

  // 调查步骤：完成一个节点后，时间块消耗在返回时不会退还。
  await page.locator(".rebirth-node-grid .rebirth-node").first().click();
  const budgetText = (await page.locator(".rebirth-investigation-head b").textContent())?.trim();

  await page.locator(".decision-step-actions .primary-action").click();
  await expect(page.locator('.decision-step-nav [aria-current="step"]')).toHaveText("日程");
  await page.locator(".decision-step-actions .secondary-action").click();
  await expect(page.locator('.decision-step-nav [aria-current="step"]')).toHaveText("调查");
  await expect(page.locator(".rebirth-investigation-head b")).toHaveText(budgetText ?? "");

  // 日程步骤：提交前可以重新选择。
  await page.locator(".decision-step-actions .primary-action").click();
  await page.locator(".focus-choice-section > .focus-grid .focus-card").nth(1).click();

  // 观点步骤：选择草案不触发结算。
  await page.locator(".decision-step-actions .primary-action").click();
  await expect(page.locator(".decision-step-actions .primary-action")).toBeDisabled();
  await page.locator(".career-option-main").first().click();
  await expect(page.locator(".career-option.is-draft-selected")).toHaveCount(1);
  await expect(page.locator(".decision-result")).toHaveCount(0);
  const historyBefore = await page.evaluate(() => {
    const envelope = JSON.parse(localStorage.getItem("rebirthSession:v1:2025") || "null");
    return envelope?.state?.history?.length ?? 0;
  });
  expect(historyBefore).toBe(0);

  // 确认步骤：返回修改可用，最终双击也只产生一条历史记录。
  await page.locator(".decision-step-actions .primary-action").click();
  await expect(page.locator(".decision-step-confirmation")).toBeVisible();
  await page.getByRole("button", { name: "返回修改" }).click();
  await expect(page.locator('.decision-step-nav [aria-current="step"]')).toHaveText("观点");
  await page.locator(".decision-step-actions .primary-action").click();

  const confirm = page.getByRole("button", { name: "确认提交本月判断" });
...
test("窄屏普通月自动跳过调查步骤", async ({ page }) => {
  ...
  for (let index = 0; index < 40; index += 1) {
    if (await page.locator(".decision-step-nav").count()) break;
    const advance = page.locator(".interaction-actions .primary-action");
    if (!(await advance.isEnabled())) break;
    await advance.click();
  }

  await expect(page.locator(".decision-step-nav li")).toHaveCount(3);
  await expect(page.locator(".decision-step-nav li").first()).toHaveText("日程");
  await expect(page.locator(".rebirth-investigation")).toHaveCount(0);
});

test("跨断点切换不会自动提交草案", async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await openCareerDecision(page);
  ...
  await expect(page.locator(".decision-step-nav")).toHaveCount(0);
  await expect(page.locator(".rebirth-investigation")).toBeVisible();
  await expect(page.locator(".decision-result")).toHaveCount(0);

  await page.setViewportSize({ width: 390, height: 844 });
  await expect(page.locator(".decision-step-nav li").first()).toBeVisible();
  await expect(page.locator(".decision-result")).toHaveCount(0);
});

test("桌面与矮横屏职业模式保持纵向决策流程", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await openCareerDecision(page);

  const sections = [
    ".stakeholder-pressure",
    ".rebirth-investigation",
    ".focus-choice-section",
    ".research-commitment",
    ".options",
  ];
  ...
  for (const selector of sections) {
    ...
    expect(box, `${selector} 应在桌面纵向流程中渲染`).not.toBeNull();
    expect(box.y, `${selector} 应位于上一个区块之下`).toBeGreaterThan(previousTop);
    previousTop = box.y;
  }
  await expect(page.locator(".decision-step-nav")).toHaveCount(0);
