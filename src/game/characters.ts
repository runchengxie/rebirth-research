...
export const CHARACTERS: Record<CharacterId, CharacterProfile> = {
  ...
  zhao_chengyu: {
    ...
    color: "slate",
    kind: "peer",
    intro: "量化组那边的同级同事，也是你在投研部最肯替你扛事的好兄弟。数据活儿稳，遇到框架对不齐基准会主动来找你商量。你在投委会被追问时，他常在旁边补一句关键数据，把你的逻辑圆回来。你卡壳，他比你还急，撸起袖子就坐过来一起拆。",
    methodology: "数据驱动，回测验证。他常说研究说得再漂亮，回测不收敛就是空中楼阁。但真到你在会上被质疑，他第一个站出来兜底，把你没说完的话接上，绝不让你一个人扛。",
  },
};
...
export const GRADE_REVIEWS: Record<MentorId, Record<string, string[]>> = {
  ...
  chen_xinghe: {
    S: ["量价信号和基本面共振！这就是我一直在说的：当大单流向和产业链逻辑指向同一个方向，Alpha 的持续性最强。", "你这次同时盯对了因子拥挤度和订单流结构。Barra 归因不会说谎，你的判断也不会。"],
    A: ["方向对了，但信号的持续性还需要更长时间窗口来确认。Alpha 衰减速度在下周会是关键。", "不错的结果！下次可以再看一眼因子正交化，排除掉那个动量因子的干扰，你的净 Alpha 会更纯。"],
