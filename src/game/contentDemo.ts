...
const CARD_CASH: KnowledgeCard = {
  ...
  mentorLine: "订单要经过回款才能变成真实收入。预收和周转拆不开，看到的只是纸面富贵。",
...
const CARD_MARGIN: KnowledgeCard = {
  ...
  mentorLine: "平价且好用的产品最终赢得了更多客户。守不住毛利的升级只是表面升级。",
...
export const THEMES_DEMO: MarketTheme[] = [
  {
    ...
    businessOutcome:
      "推理单价年内降了约一个数量级，原本犹豫的小客户开始规模化接入。这家公司没有守住入口，份额被更底层的基础设施方吃掉了。生意的变化远比账面涨跌复杂。",
    competingHypotheses: {
  ...
  },
  {
    ...
    businessOutcome:
      "订单要经过回款才能变成真实收入。重视认证和本地服务的公司把账做成了现金，只拼出厂价的公司卡在应收账款里。框架比方向诚实。",
    competingHypotheses: {
  ...
  },
  {
    ...
    protagonistMemory:
      "你记得后来平价且好用的产品赢得了更多客户，供应链效率高的公司吃下了中间市场。",
    ...
    businessOutcome:
      "份额向效率型平价品牌集中。消费并未消失，能控住周转和损耗的企业把降价做成了护城河。",
    competingHypotheses: {
  ...
  },
  {
    ...
    protagonistMemory:
      "你记得后来真正拉开差距的是多条产线能否稳定运行，以及良率爬坡速度。单次做出样品无法决定胜负。",
...
function pool0(): ResearchDecision[] {
  return [
    ...
    d({
      ...
      label: "准时下班，脑子留干净",
      category: "self_care",
      description: "连轴转会让信噪比下降。充分休息能给模型留出干净输入。",
...
function buildDemoCompetingNode(monthIndex: number): SceneNode {
  return {
...
export function buildDemoChapter(monthIndex: number, state?: GameState): MonthScene {
  ...
  const competingNode: SceneNode | null =
    state && theme.competingHypotheses ? buildDemoCompetingNode(monthIndex) : null;
